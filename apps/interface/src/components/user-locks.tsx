...
export function UserLocks({ autoFetch = true }: UserLocksProps) {
  ...
  useEffect(() => {
    if (!boardAddress || initiativesInitialized) return
    void fetchInitiatives(boardAddress)
  }, [boardAddress, initiativesInitialized, fetchInitiatives])

  // Fetch user locks when component mounts and user is connected
  useEffect(() => {
    if (!autoFetch) return
    if (!address) {
      setUserLocks([])
    ...
    void fetchUserLocks()
  }, [autoFetch, address, fetchUserLocks])
