...
contract SignalsHarness is Test, Deployers {
    ...
    struct SignalsConfig {
        ...
        uint8 decayCurveType;
        uint256[] decayCurveParameters;
    }

    SignalsConfig public defaultConfig = SignalsConfig({
        owner: _deployer,
    ...
    // Contract that implements the IBondIssuer interface for testing
    contract MockIssuer is IBondIssuer {
        ...
        function transferFrom(address from, address to, uint256 tokenId) external {
            // Simple mock implementation that just transfers the token
            // In a real implementation this would check ownership etc.
        }
    ...
    function deployMockIssuer() public returns (MockIssuer) {
        mockIssuer = new MockIssuer(address(_token));
        _dealDefaultTokens();
        return mockIssuer;
    }
    ...
    function deployHookWithLiquidity(IBondIssuer _issuer) public {
        ...
        deployCodeTo("BondHook.sol", abi.encode(manager, address(_issuer), address(pricing)), address(flags));
