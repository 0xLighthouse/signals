...
contract ModifyLiquidityTest is Test, Deployers, BondHookHarness {
    ...
    function test_earnFees() public {
        ...
        vm.stopPrank();
        assertApproxEqAbs(_dai.balanceOf(address(_liquidityProvider)), _initialDaiBalance + 2_500 ether, 1000, "Incorrect dai balance");
        assertApproxEqAbs(_token.balanceOf(address(_liquidityProvider)), _initialTokenBalance + 2_500 ether, 1000, "Incorrect token balance");
    }
}