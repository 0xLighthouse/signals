...
**Critical timing:** This must be called before the `boardOpensAt` timestamp. Once the board opens, you cannot change the incentives pool.
...
|-------|-------|----------|
| Cannot set pool | Board already opened | Must set before `boardOpensAt` |
