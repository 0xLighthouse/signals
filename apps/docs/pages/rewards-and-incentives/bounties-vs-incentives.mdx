...
// Where:
// t = (lockCreationTime - boardOpenAt) / (acceptanceTime - boardOpenAt)
...
```solidity
// Rewards tracked internally by board using bucket system
// Pool is called when supporters redeem to pay rewards
...
**Incentives:**
```solidity
// Configuration provided when setting pool (before board opens)
