...
**Important:** Time is measured from the **board open time** (`boardOpenAt`), not individual initiative creation times. This ensures fair participation and prevents gaming.
...
To prevent gaming and ensure equitable participation, boards have a scheduled **opening time** configured via `boardOpenAt`:
...
struct BoardConfig {
    // ... existing fields
    uint256 boardOpenAt;  // Timestamp when board opens for participation
}
```
...
**Implementation:**
- `proposeInitiative()` and `supportInitiative()` require `block.timestamp >= boardOpenAt`
- Incentives calculation uses `boardOpenAt` as `t=0`, not initiative creation time
...
struct BoardConfig {
    // ... existing fields
    uint256 boardOpenAt;  // When board opens
}
```
...
7. Signals tracks rewards using bucket-based system internally
8. Incentivizer (board mixin):
   - Calculates time weights using 24 time buckets
   - Alice & Bob: ~100 weight (both supported immediately)
   - Charlie: 94 weight (supported at t=0.5)
   - Determines reward percentages for each supporter
...
1. **Set Pool Before Opening**: Call `setIncentivesPool()` before `boardOpenAt`
2. **Clear Communication**: Announce board opening time with timezone
3. **Fair Configuration**: Use reasonable `boardOpenAt` to give community time
...
A: Check if `signals.incentivesPool()` returns a non-zero address.
