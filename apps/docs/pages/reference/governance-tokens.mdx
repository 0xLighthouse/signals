...
Configure your board to require a `minHoldingDuration` in `proposerRequirements` and set the `token` field to your governance token (it can differ from the underlying lock token).
...
function meetsProposalRequirements() internal view returns (bool) {
    ...
    if (reqs.minHoldingDuration > 0) {
        ...
        uint256 pastBalance = IVotes(reqs.token).getPastVotes(
            msg.sender,
            pastBlock
        );
