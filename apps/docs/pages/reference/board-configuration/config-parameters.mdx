...
struct BoardConfig {
    ...
    uint256 opensAt;                              // Unix timestamp for opening
    uint256 closesAt;                            // Unix timestamp for closing (0 = never)
}
```
...
- **Values:**
  - `0` - Board stays closed until explicitly opened via `setOpensAt`
...
| `Signals_InvalidArguments` | closesAt < opensAt | Ensure close time is after open time |
