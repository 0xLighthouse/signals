...
Actioning an initiative means officially approving it, transitioning it from Proposed to Accepted state. This triggers incentive calculation and makes locked tokens redeemable.
...
If an incentives pool is configured, rewards are calculated based on each supporter's time-weighted contribution. Supporters claim rewards when they redeem their locks.
