...
1. **Board is open** - Current time ≥ `boardOpenAt` and board state is Open
...
Boards configure participant requirements using three fields (`minBalance`, `minHoldingDuration`, `minLockAmount`). Different patterns emerge based on which fields are set:
