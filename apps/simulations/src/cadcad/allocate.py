...
def allocate_tokens(
    ...
    if randomize:
        ...
        if num_users > 0:
            ...
            print(f"Warning: {total_supply} tokens to distribute but num_users is 0 in randomize mode.")

    elif (
        distribution_rule
        and len(distribution_rule) == 2
        and 0 < distribution_rule[0] < 100
        and 0 <= distribution_rule[1] <= 100
        and num_users > 0
    ):
        percent_users_control = distribution_rule[0]
        percent_tokens_control = distribution_rule[1]
        ...
        num_control_users = min(num_users, max(1, round(num_users * (percent_users_control / 100.0))))
        ...
        if num_other_users > 0:
        ...
        elif tokens_for_other_group > 0:  # No other users, but tokens remain (should be rare if logic is correct)
            ...
            if control_user_ids:  # Ensure there is at least one user to give the remainder to
                ...
                print(f"Warning: Remaining tokens {tokens_for_other_group} but no users to assign them to.")
        ...
        if num_users > 0:
            ...
            print(f"Warning: {total_supply} tokens to distribute but num_users is 0 in default mode.")
