...
- [ ] **BUDG-02**: Fix `_gini()` to return `np.nan` for zero-sum arrays instead of `0.0` *(Research: P4 — returns false "perfect equality" for zero-participation proposals, corrupts sweep heatmaps — PITFALLS.md)*
- [ ] **BUDG-03**: Fix `_enp()` to return `np.nan` for zero-weight arrays instead of `0.0` *(Research: P4 — ENP=0 is outside valid range [1, n_voters], corrupts sweep edge cases — PITFALLS.md)*
